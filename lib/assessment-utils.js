...
export function assessCriterion({ criterion, caseData, transcript, revealedFactIds = [] }) {
  ...
  const actionTriggers = (criterion.studentActionTriggers || []).map(normalise).filter(Boolean);
  const isStudentAction = actionTriggers.length > 0;
  const facts = (criterion.factIds || []).map((id) => caseData.facts.find((fact) => fact.id === id)).filter(Boolean);
  const actionEvidence = isStudentAction
    ? turns.find((turn) => actionTriggers.some((trigger) => normalise(turn.content).includes(trigger))) || null
    : null;
  const directEvidence = isStudentAction
    ? actionEvidence
    : facts.map((fact) => findEvidenceForFact(fact, turns)).find(Boolean) || null;
  const partialEvidence = isStudentAction
    ? null
    : directEvidence || facts.map((fact) => findPartialEvidence(fact, turns)).find(Boolean) || null;
  const wasRevealed = isStudentAction ? false : facts.some((fact) => revealedFactIds.includes(fact.id));
  ...
  const q = isStudentAction
    ? { level:'good', flags:[], comment:'此評量項目依學生實際提供的說明或衛教內容判定。' }
    : questionQuality((directEvidence || partialEvidence)?.content || '');
  let ratio = status === 'covered' ? 1 : status === 'partial' ? 0.5 : 0;
  if (!isStudentAction && status === 'covered' && q.flags.includes('leading')) ratio = 0.8;
  ...
  return {
    ...
    reasoning:isStudentAction
      ? (status === 'covered'
          ? '完整問診紀錄中找到學生實際提供此項說明或衛教的內容。'
          : '完整問診紀錄中未找到學生實際提供此項說明或衛教的內容。')
      : status === 'covered'
