export const cases = {
  ...
  voice_nodule_001: {
    ...
    responseRules: [
      { id:'voice_rest_advice', when:'學生談到減少大聲說話、增加聲音休息、改善下課管理或善用擴音', response:'下課都急著處理學生，有時候真的會不自覺大聲，這個可能要改一下。' },
      { id:'hydration_advice', when:'學生建議增加喝水或補充水分', response:'所以是一次要喝很多水嗎？可是我不喜歡喝白開水，可以用咖啡或茶代替嗎？' },
      { id:'throat_clear_advice', when:'學生說明應減少清喉嚨', response:'我真的很常不自覺就想清喉嚨，這個習慣可能要慢慢改。' },
      { id:'nodule_explanation', when:'學生向病人解釋聲帶結節或形成原因', response:'聲帶長繭以後是不是就不會恢復了？我才教書第二年而已，現在就長繭會不會太快了？' },
      { id:'reassured', when:'學生適當回應病人對恢復與保養的疑問', response:'好，那看來我要很認真保養喉嚨。' }
    ],
    facts: [
...
export function isUsableCaseDefinition(value) {
  return Boolean(
    ...
    (!value.responseRules || (Array.isArray(value.responseRules) && value.responseRules.every((rule) =>
      rule && typeof rule.id === 'string' && typeof rule.when === 'string' && typeof rule.response === 'string'
    ))) &&
