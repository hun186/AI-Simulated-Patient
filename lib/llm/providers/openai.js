...
export async function generateOpenAI(request,{fetchImpl=fetch}={}){
  ...
  const requestedServiceTier=String(request.serviceTier||'default').toLowerCase();
  if(['default','flex','fast','priority'].includes(requestedServiceTier)){
  ...
  return {
    ...
    serviceTier:String(data.service_tier||requestedServiceTier||'default'),
    usage:usageOf(data),
