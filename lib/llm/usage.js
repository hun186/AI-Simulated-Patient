...
export async function recordLlmUsage({
  ...
  const pricingRule=await resolvePricingRule({preset,model});
  ...
  await query(
    `insert into llm_usage_events
      (user_id,session_id,case_id,agent_type,connection_id,provider_kind,preset,model,
       ...
       estimated_cost_microusd,pricing_status,pricing_rule_id)
    ...
    [
      ...
      pricing.estimatedCostMicrousd,pricing.pricingStatus,pricing.pricingRuleId
    ]
  );
}
