const PUBLIC_MESSAGES={
  ...
  insufficient_balance:'LLM provider account has insufficient balance',
...
export function errorForStatus(status){
  ...
  if(status===400||status===422) return new LlmProviderError('invalid_request',{status});
  if(status===402) return new LlmProviderError('insufficient_balance',{status});
