...
export async function usageSummary(actor,{userId=null,from=null,to=null}={}){
  ...
  if(!ids.length) return {users:visible,totals:[],byDate:[],byProvider:[],byModel:[],byAgent:[],fxRate:await resolveFxRate()};
  ...
  const [totals,byDate,byProvider,byModel,byAgent]=await Promise.all([
    query(
      `select user_id as "userId",count(*) as calls,
       ...
       coalesce(sum(case when estimated_cost_microntd is not null then estimated_cost_microntd else 0 end),0) as "estimatedCostMicrontd",
  ...
  return {users:visible,totals,byDate,byProvider,byModel,byAgent,fxRate:await resolveFxRate()};
}
