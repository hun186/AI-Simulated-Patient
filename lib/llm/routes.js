...
async function existingRoute(scopeType,scopeId,agentType){
  const rows=await query(
    `select r.*,c.provider_kind,c.preset
     ...
     where r.scope_type=$1
       ...
       and r.agent_type=$3
     limit 1`,
    [scopeType,scopeId,agentType]
  );
  return rows[0]||null;
}

async function saveRoute({scopeType,scopeId=null,agentType,connection,model,config,actor}){
  ...
  const existing=await existingRoute(scopeType,scopeId,agentType);
  if(existing){
    await query(
      `update llm_agent_routes
       set connection_id=$2,model=$3,config_json=$4::jsonb,created_by=$5,updated_at=now()
       where id=$1`,
      [existing.id,connection.id,chosenModel,configJson,actor.id]
    );
  ...
  await query(
    `insert into llm_agent_routes
      (id,scope_type,scope_id,agent_type,connection_id,model,config_json,created_by)
     values ($1,$2,$3,$4,$5,$6,$7::jsonb,$8)`,
    [id,scopeType,scopeId,agentType,connection.id,chosenModel,configJson,actor.id]
  );
  return routeById(id);
}
...
export async function setSystemRoute(actor,{agentType,connectionId,model=null,config={}}){
  ...
  return saveRoute({
    scopeType:'system',scopeId:null,agentType,connection,model,config,actor
  });
}
...
export async function setCaseRoute(actor,{caseId,agentType,connectionId,model=null,config={}}){
  ...
  if(actor.role===ROLE_ADMIN){
    ...
    if(foundCase.created_by!==actor.id) throw fail('FORBIDDEN_CASE');
  ...
  return saveRoute({
    scopeType:'case',scopeId:caseId,agentType,connection,model,config,actor
  });
}
...
export async function deleteRoute(actor,routeId){
  ...
  if(actor?.role!==ROLE_ADMIN){
    if(actor?.role!==ROLE_TEACHER || route.scope_type!=='case' || route.created_by!==actor.id){
      throw fail('FORBIDDEN');
    }
    const foundCase=await caseRow(route.scope_id);
    if(!foundCase || foundCase.created_by!==actor.id) throw fail('FORBIDDEN_CASE');
  }
  await query('delete from llm_agent_routes where id=$1',[routeId]);
}

export async function resolveAgentRoutes({caseId}){
  await ensureBuiltinCase();
  const rows=await query(
    `select r.*,c.provider_kind,c.preset
     ...
     where c.is_active=true
       and (r.scope_type='system' or (r.scope_type='case' and r.scope_id=$1))
     order by case when r.scope_type='case' then 0 else 1 end, r.updated_at desc`,
    [caseId]
  );
    ...
    rows=await query(
      `select r.*,c.provider_kind,c.preset
       ...
       where r.scope_type='system'
          or (r.scope_type='case' and exists (
            select 1 from cases x where x.id=r.scope_id and x.created_by=$1
          ))
       order by r.updated_at desc`,
      [actor.id]
    );
