...
export async function consumeAttempt(req,scope,identifier,{limit=20,windowSeconds=3600,blockSeconds=900}={}){
  limit=Math.max(1,Number(limit)||20);
  windowSeconds=Math.max(60,Number(windowSeconds)||3600);
  blockSeconds=Math.max(60,Number(blockSeconds)||900);
  const host=clientHost(req);
  const key=keyHash(scope,identifier,host);
  const rows=await query('select failures,window_started_at,blocked_until from auth_throttle where key_hash=$1 limit 1',[key]);
  const now=new Date();
  const row=rows[0];
  if(row?.blocked_until){
    const blockedUntil=new Date(row.blocked_until).getTime();
    if(blockedUntil>now.getTime()) return Math.max(1,Math.ceil((blockedUntil-now.getTime())/1000));
  }
  let attempts=1;
  let windowStarted=now;
  if(row){
    const started=new Date(row.window_started_at);
    if(Number.isFinite(started.getTime()) && now.getTime()-started.getTime()<=windowSeconds*1000){
      attempts=Number(row.failures||0)+1;
      windowStarted=started;
    }
  }
  const blockedUntil=attempts>limit?new Date(now.getTime()+blockSeconds*1000):null;
  await query(
    `insert into auth_throttle (key_hash,scope,failures,window_started_at,blocked_until,updated_at)
     values ($1,$2,$3,$4,$5,now())
     on conflict (key_hash) do update set failures=excluded.failures,window_started_at=excluded.window_started_at,
       blocked_until=excluded.blocked_until,updated_at=now()`,
    [key,scope,attempts,windowStarted.toISOString(),blockedUntil?.toISOString()||null]
  );
