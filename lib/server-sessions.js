...
export async function createInterviewSession({user,caseId,mode='training',coachEnabled=false}) {
  ...
  await query(
    `insert into interview_sessions
      (id,case_id,case_version,student_user_id,mode,coach_enabled,coach_used,status,revealed_fact_ids)
     values ($1,$2,$3,$4,$5,$6,$6,'active','[]'::jsonb)`,
...
export async function updateCoachEnabled(sessionId,user,enabled) {
  const rows=await query(
    `update interview_sessions
     set coach_enabled=$4,coach_used=(coach_used or $4),updated_at=now()
