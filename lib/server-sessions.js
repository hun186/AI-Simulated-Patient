...
export async function createInterviewSession({user,caseId,mode='training',coachEnabled=false}) {
  ...
  await query(
    `insert into interview_sessions
      (id,case_id,case_version,case_snapshot,student_user_id,mode,coach_enabled,coach_used,status,revealed_fact_ids)
     values ($1,$2,$3,$4::jsonb,$5,$6,$7,$7,'active','[]'::jsonb)`,
    [id,caseId,found.row.version,JSON.stringify(found.definition),user.id,mode,Boolean(coachEnabled)]
  );
  await query(
