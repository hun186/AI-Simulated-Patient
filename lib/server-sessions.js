...
export async function createInterviewSession({user,caseId,mode='training',coachEnabled=false}) {
  ...
  const routeOwnerUserId=user?.role==='teacher'?user.id:null;
  const resolved=await resolveAgentRoutes({caseId,routeOwnerUserId});
  if(isProductionEnv() && !resolved.patient) throw providerError('AI_PROVIDER_NOT_CONFIGURED');
  if(isProductionEnv() && !resolved.evaluator) throw providerError('AI_EVALUATOR_PROVIDER_NOT_CONFIGURED');
  ...
  return {
    id,caseId,mode,coachEnabled:Boolean(coachEnabled),opening:found.definition.opening,
    runtime:{
      patient:routeSnapshot.patient?{
        providerKind:routeSnapshot.patient.providerKind,
        preset:routeSnapshot.patient.preset,
        model:routeSnapshot.patient.model
      }:null,
      coach:routeSnapshot.coach?{
        providerKind:routeSnapshot.coach.providerKind,
        preset:routeSnapshot.coach.preset,
        model:routeSnapshot.coach.model
      }:null,
      evaluator:routeSnapshot.evaluator?{
        providerKind:routeSnapshot.evaluator.providerKind,
        preset:routeSnapshot.evaluator.preset,
        model:routeSnapshot.evaluator.model
      }:null
    }
  };
}
