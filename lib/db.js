...
async function adapter(){
  ...
  cachedDriver=driver;
  if(driver==='sqlite'){
    // Keep the native SQLite adapter out of Vercel's serverless bundle tracing.
    // Local Node resolves this computed specifier normally when SQLite is selected.
    const sqliteModule='./db-'+'sqlite.js';
    cachedAdapter=await import(sqliteModule);
  }else{
    cachedAdapter=await import('./db-postgres.js');
  }
