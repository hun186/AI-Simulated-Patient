...
async function renderRecords(){if(state.serverMode){if(state.user?.role!=='teacher')return;try{
  ...
  state.records=rows.map(r=>({...r,percentage:Number(r.percentage),completedAt:r.endedAt||r.startedAt,coachUsed:r.coachEnabled,evaluation:r.evaluation||{percentage:Number(r.percentage),items:[]},transcript:r.transcript||[]}));
