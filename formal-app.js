...
async function renderRecords(){if(state.serverMode){if(state.user?.role!=='teacher')return;try{
  ...
  state.records=rows.map(r=>({...r,completedAt:r.endedAt||r.startedAt,coachUsed:r.coachEnabled,evaluation:r.evaluation||{percentage:r.percentage,items:[]},transcript:r.transcript||[]}));
