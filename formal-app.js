...
function save(){write(SKEY,{caseId:state.caseId,mode:state.mode,studentName:state.studentName,transcript:state.transcript,revealedFactIds:state.revealedFactIds,sessionId:state.sessionId,coach:state.coach,coachEnabled:state.coachEnabled,coachUsed:state.coachUsed});}
...
async function ask(q){state.transcript.push({role:'student',content:q,at:new Date().toISOString()});renderChat();$('sendBtn').disabled=true;try{const d=await post('/api/chat',{caseId:state.caseId,caseDefinition:currentCaseDefinition(),message:q,revealedFactIds:state.revealedFactIds});state.revealedFactIds=d.revealedFactIds;state.transcript.push({role:'patient',content:d.reply,at:new Date().toISOString()});if(state.mode==='training'&&state.coachEnabled){state.coach=await post('/api/coach',{caseId:state.caseId,caseDefinition:state.serverMode?null:currentCaseDefinition(),sessionId:state.serverMode?state.sessionId:null,transcript:state.transcript,revealedFactIds:state.revealedFactIds});state.coachUsed=true;}save();renderChat();renderMode();renderCoach();}catch{state.transcript.push({role:'patient',content:'（系統暫時無法取得回覆。）'});renderChat();}finally{$('sendBtn').disabled=false;}}
...
async function setCoachEnabled(enabled){if(state.mode!=='training'){state.coachEnabled=false;$('coachToggle').value='off';return;}state.coachEnabled=enabled;if(!enabled){state.coach=null;save();renderMode();renderCoach();return;}state.coachUsed=true;const hasQuestion=state.transcript.some(m=>m.role==='student');if(hasQuestion){try{state.coach=await post('/api/coach',{caseId:state.caseId,caseDefinition:state.serverMode?null:currentCaseDefinition(),sessionId:state.serverMode?state.sessionId:null,transcript:state.transcript,revealedFactIds:state.revealedFactIds});}catch{state.coach=null;}}save();renderMode();renderCoach();}
